                ...
                <div className="flex flex-col gap-2 p-3">
                    <div className="flex flex-row gap-2 items-center">
                        ...
                        <div className="flex flex-row gap-1">
                            {project.badges && project.badges.map((badge, index) => <Badge key={index} variant={badge.variant} className={badge.className}>{badge.text}</Badge>)}
                        </div>
                    </div>
                    <div className={clsx({ "italic": !project.description }, "text-secondary-foreground text-sm font-regular")}>{project.description ?? "No description provided."}</div>
                </div>
                {(project.sourceLink || project.projectLink) &&
                    <div className="w-full flex flex-row items-end justify-end gap-2">
                        {project.sourceLink && <Button size={"sm"} variant={"outline"} asChild><a href={project.sourceLink}>Source code<Code2Icon/></a></Button>}
                        {project.projectLink && <Button size={"sm"} asChild><a href={project.projectLink}>Go to Project <GlobeIcon/></a></Button>}
                    </div>
                }
            </div>
        </div>
